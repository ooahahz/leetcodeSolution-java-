...
class Solution {
    public int strongPasswordChecker(String password) {
        ...
        if (n < 6) {
            ...
            // k mod 3 = 1 的组数，即删除2个字符可以减少哦1次替换操作
            int rm2 = 0;
            int cnt = 0;
            char cur = '#';
            ...
            replace += cnt / 3;
                    // 使用 k % 3 的组的数量，由剩余的替换次数、组数和剩余的删除次数共同决定
            int use2 = Math.min(Math.min(replace, rm2), remove / 2);
            replace -= use2;
            remove -= use2 * 2;
            // 由于每一次替换次数就一定有3个连续相同的字符（k/3决定），因此这里可以直接计算出使用 k % 3 = 2 的组的数量
